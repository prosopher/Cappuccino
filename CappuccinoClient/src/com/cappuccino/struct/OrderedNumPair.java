...
public class OrderedNumPair<N extends Number> extends GeneralPair<N, N> implements Comparable<OrderedNumPair<N>> {
	...
	public int compareTo(OrderedNumPair<N> that) {
		int result = new BigDecimal(this.first.toString()).compareTo(new BigDecimal(that.first.toString()));
		return result == 0 ? new BigDecimal(this.second.toString()).compareTo(new BigDecimal(that.second.toString()))
				: result;
	}
